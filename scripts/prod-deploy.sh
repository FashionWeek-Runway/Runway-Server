...
echo ">>> build 파일명: $JAR_NAME" >> /var/www/prod/deploy.log

echo ">>> build 파일 복사" >> /var/www/prod/deploy.log
DEPLOY_PATH=/var/www/prod/build/libs/
cp $BUILD_JAR $DEPLOY_PATH

echo ">>> 현재 실행중인 애플리케이션 pid 확인" >> /var/www/prod/deploy.log
...
if [ -z $CURRENT_PID ]
then
  echo ">>> 현재 구동중인 애플리케이션이 없으므로 종료하지 않습니다." >> /var/www/prod/deploy.log
else
  #kill -15 $CURRENT_PID
  fuser -k 8000/tcp
  echo ">>> kill -15 $CURRENT_PID" >> /var/www/prod/deploy.log
  sleep 5
fi
